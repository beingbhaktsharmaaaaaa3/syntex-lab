...
-- Ensure idempotent seed: add unique constraints where missing
ALTER TABLE hall_of_fame      ADD CONSTRAINT IF NOT EXISTS uq_hof_handle    UNIQUE (handle);
ALTER TABLE vuln_flags        ADD CONSTRAINT IF NOT EXISTS uq_flag_slug      UNIQUE (slug);
ALTER TABLE challenge_completions ADD CONSTRAINT IF NOT EXISTS uq_completion UNIQUE (user_id, challenge_id);
