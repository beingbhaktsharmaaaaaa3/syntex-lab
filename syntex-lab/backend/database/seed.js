...
// ── v3.1 seed — Flag definitions ─────────────────────────────────
async function seedFlags(client) {
    const flags = [
        {
            slug: 'sqli-login',
            flag: 'FLAG{sqli_auth_bypass_admin_owned_a1b2c3d4}',
            title: 'SQL Injection — Login Bypass',
            category: 'SQLi', points: 250,
            hint: "The admin user's private notes contain this flag. Bypass auth to read them.",
        },
        {
            slug: 'idor-profile-user2',
            flag: 'FLAG{idor_john_doe_profile_exposed_e5f6g7h8}',
            title: 'IDOR — User 2 Profile',
            category: 'IDOR', points: 150,
            hint: "User #2's secret_note field. Read it without being user #2.",
        },
        {
            slug: 'idor-profile-admin',
            flag: 'FLAG{idor_admin_api_key_and_notes_i9j0k1l2}',
            title: 'IDOR — Admin Profile via API',
            category: 'IDOR', points: 200,
            hint: "GET /api/v1/users/1 — admin's secret_note without authentication.",
        },
        {
            slug: 'idor-orders',
            flag: 'FLAG{idor_order6_license_and_notes_m3n4o5p6}',
            title: 'IDOR — Order #6 Notes',
            category: 'IDOR', points: 150,
            hint: "Order #6 notes field. Access it as any user other than the order owner.",
        },
        {
            slug: 'idor-tickets',
            flag: 'FLAG{idor_ticket3_internal_staff_notes_q7r8s9t0}',
            title: 'IDOR — Ticket #3 Internal Notes',
            category: 'IDOR', points: 150,
            hint: "Ticket #3 internal_notes. Only staff should see this — but any user can.",
        },
        {
            slug: 'exposed-env',
            flag: 'FLAG{dotenv_file_publicly_served_u1v2w3x4}',
            title: 'Exposed .env File',
            category: 'Exposure', points: 100,
            hint: "A configuration file served at a well-known path. No authentication required.",
        },
        {
            slug: 'debug-page',
            flag: 'FLAG{debug_endpoint_dumps_all_env_vars_y5z6a7b8}',
            title: 'Debug Page — Full Env Dump',
            category: 'Exposure', points: 100,
            hint: "A debug endpoint returns every environment variable including secrets.",
        },
        {
            slug: 'internal-config-api',
            flag: 'FLAG{api_v2_internal_config_no_auth_c9d0e1f2}',
            title: 'Internal Config API — No Auth',
            category: 'Broken Access', points: 200,
            hint: "An /api/v2 endpoint returns the full app config. No token needed.",
        },
        {
            slug: 'mass-user-export',
            flag: 'FLAG{unauthenticated_user_export_hashes_g3h4i5j6}',
            title: 'Unauthenticated User Export',
            category: 'Broken Access', points: 150,
            hint: "An API endpoint exports all users with MD5 hashes. Zero authentication required.",
        },
        {
            slug: 'admin-panel-bypass',
            flag: 'FLAG{admin_panel_cookie_role_bypass_k7l8m9n0}',
            title: 'Admin Panel Bypass via Cookie',
            category: 'Auth Bypass', points: 200,
            hint: "The admin panel checks something client-side. Find it in browser DevTools.",
        },
        {
            slug: 'developer-secrets',
            flag: 'FLAG{dev_account_jwt_and_db_secret_o1p2q3r4}',
            title: 'Developer Account Secret Note',
            category: 'IDOR', points: 200,
            hint: "A developer account's private notes contain JWT_SECRET and DB_PASS.",
        },
        {
            slug: 'git-config-token',
            flag: 'FLAG{git_config_embedded_deploy_token_s5t6u7v8}',
            title: 'Exposed .git/config with Token',
            category: 'Exposure', points: 100,
            hint: "The git configuration file is publicly accessible and contains credentials.",
        },
        {
            slug: 'jwt-algnone',
            flag: 'FLAG{jwt_alg_none_no_signature_check_w9x0y1z2}',
            title: 'JWT Algorithm Confusion — alg:none',
            category: 'JWT', points: 250,
            hint: "A pre-built admin JWT lives in a public JS file. The server accepts it.",
        },
        {
            slug: 'ssrf-internal',
            flag: 'FLAG{ssrf_internal_network_reachable_a3b4c5d6}',
            title: 'SSRF — Internal Network Access',
            category: 'SSRF', points: 250,
            hint: "Use the URL fetch endpoint to reach a page only accessible from inside the server.",
        },
        {
            slug: 'xss-stored-comments',
            flag: 'FLAG{stored_xss_blog_comments_executed_e7f8g9h0}',
            title: 'Stored XSS — Blog Comments',
            category: 'XSS', points: 200,
            hint: "Post a comment that executes JavaScript. The flag appears in the browser console when it fires.",
        },
    ];

    for (const f of flags) {
        await client.query(
            `INSERT INTO vuln_flags (slug, flag_value, vuln_title, category, points, location_hint)
             VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (slug) DO UPDATE
             SET flag_value=$2, vuln_title=$3, category=$4, points=$5, location_hint=$6`,
            [f.slug, f.flag, f.title, f.category, f.points, f.hint]
        );
    }

    // Update user secret_notes to use canonical flag values
    await client.query(`UPDATE users SET secret_note=$1 WHERE id=1`,
        ['FLAG{idor_admin_api_key_and_notes_i9j0k1l2} | api_key: sk_admin_8f3a2b1c9d4e5f6a7b8c9d0e1f2a3b4c']);
    await client.query(`UPDATE users SET secret_note=$1 WHERE id=2`,
        ['FLAG{idor_john_doe_profile_exposed_e5f6g7h8}']);
    await client.query(`UPDATE users SET secret_note=$1 WHERE id=6`,
        ['FLAG{dev_account_jwt_and_db_secret_o1p2q3r4} | JWT_SECRET=secret123 | DB_PASS=Synx@2024!Prod']);
    await client.query(`UPDATE orders  SET notes=$1 WHERE id=6`,
        ['FLAG{idor_order6_license_and_notes_m3n4o5p6}']);
    await client.query(`UPDATE tickets SET internal_notes=$1 WHERE id=3`,
        ['FLAG{idor_ticket3_internal_staff_notes_q7r8s9t0} | Issue: EU VAT calculation bug confirmed.']);

    console.log('[SEED v3.1] Flags seeded and planted.');
}

module.exports = { seedV3, seedFlags };
